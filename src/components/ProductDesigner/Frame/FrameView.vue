<template>
  <el-card>
    <el-header>
      <h1 v-if="$route.path === '/frame/' + $route.params.id">編輯</h1>
      <h1 v-else>檢視</h1>
    </el-header>
    ...
    >
      ...
      >
        ...
        <el-row>
          <el-col :span="4">
            <div v-if="$route.path === '/frame/' + $route.params.id">
              <el-select v-model="domain.materialid" placeholder="請選擇">
                <el-option
          ...
          <el-col :span="4">
            <div style="text-align:right">
              <el-button
                v-if="$route.path === '/frame/' + $route.params.id"
...
export default {
  ...
  computed: {
    // test () {
    //   return this.materialdata.filter(material => material.materialid == this.dynamicValidateForm.domains.materialid
    //   )
    // }
  },
  ...
  methods: {
    test(row) {
      ...
      this.materialdata.forEach(element => {
        console.log(element);
        // console.log(row.materialid == element.materialid)
        if (row.materialid == element.materialid) {
          console.log("OK");
          a = element.cost;
        }
      });
