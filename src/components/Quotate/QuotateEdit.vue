<template>
  <div>
    <h1 style="text-align:center;">編輯報價單</h1>
    <el-card>
    ...
    <el-card>
      <div>
        ...
        <br />
        <el-button type="info" @click="submitForm('postQuotateForm')"
          >提交</el-button
        >
      ...
      this.$confirm("此操作將永久刪除該筆資料, 是否繼續?", "提示", {
        ...
        .then(res => {
          ...
          if (res == "confirm") {
            console.log("ok");
            this.axios({
              url: "/reportdetail/deleteall",
              method: "delete",
              data: data
            }).then(res => {
              if (res.status == 204) {
                var index = this.dynamicValidateForm.domains.indexOf(row);
