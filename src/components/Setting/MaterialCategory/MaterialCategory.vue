  ...
  <div>
    ...
    <el-card style="width: 100%">
      ...
      >
        ...
        text-align: center;"
        >
          <i class="bi bi-plus-square"></i>材料種類設定
        </h1>
      </div>
    ...
    >
      <el-form>
        ...
        <el-form-item label="種類" prop="category" label-width="100px">
...
export default {
  ...
  methods: {
      ...
      this.$confirm("此操作將永久刪除該筆資料, 是否繼續?", "提示", {
        ...
        .then(res => {
          ...
          if (res == "confirm") {
            ...
            //delete API請求
            this.axios({
              url: `/materialcategory/${row.materialcategoryid}`,
              method: "delete"
            })
              .then(res => {
                console.log(res);
    ...
    postmeterialapi() {
      ...
      if (this.title == "新增") {
        //   this.axios({
        //     url: `/findmaterialcategory/${this.editform.materialcategoryid}`,
        //     method: "get"
        //   }).then(res => {
        //     if (res.data == false) {
        //       console.log("ok");
    ...
    putmeterialapi(id) {
      this.axios({
        ...
        .then(res => {
          ...
          this.$message.success({ message: "成功新增資料" });
          this.editDialogVisible = false;
        })
        .catch(err => {
          console.log(err);
