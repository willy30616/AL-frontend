    ...
    <el-card style="width: 100%">
      ...
      >
        <el-button
          ...
          type="success"
          icon="el-icon-plus"
          @click="$router.push('/doorleaf/Add')"
          >新增門扇</el-button
        >
        <h1 style="width:100% ;text-align: center;">
          <i class="bi bi-door-open-fill"></i>門扇設定
        </h1>
      </div>
      <br />
      ...
      >
        ...
        <el-table-column prop="specid" label="門扇名稱(規格)">
        </el-table-column>
        ...
        <el-table-column label="操作" width="100px">
          ...
          <template slot-scope="scope">
            <router-link :to="`/doorleaf/view/${scope.row.specid}`">
...
export default {
  ...
  methods: {
    ...
    editDialog(row) {
      ...
      console.log(row);
      this.$router.push(`/doorleaf/${row.specid}`);
    },

    //刪除對話視窗
    //先用 (/findspec/{specid}) 這隻API 搜尋specid
    //如果返回值為空 此規格(specid)就可被刪
    //反之 就不能刪除
    deleteDialog(row) {
